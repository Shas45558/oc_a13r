 ...
 * (FUNC_LV_MODULE | FUNC_LV_CPUFREQ | \
...
#define cpufreq_write_mask(addr, mask, val) \
	cpufreq_write(addr, \
	(cpufreq_read(addr) & ~(_BITMASK_(mask))) | _BITS_(mask, val))
...
#define cpu_dvfs_get_max_volt(p)				\
...
extern void set_cur_freq_wrapper(struct mt_cpu_dvfs *p,
